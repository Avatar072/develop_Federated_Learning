...
def LabelEncodingAndReplaceMorethanTenthousandQuantity(df):
    ...
    # df['DestinationPort'] = df['DestinationPort'].astype(str)# 這邊Label encoding失敗，DestinationPort拉出來單獨從做一次
    # label_Encoding('DestinationPort',df)
    # 保存編碼后的 DataFrame 回到 CSV 文件
    df.to_csv(filepath + "\\onlyThursday\\total_encoded_Thursday.csv", index=False)
  
    # 超過提取10000行的只取10000，其餘保留 
    df_temp = pd.read_csv(filepath + "\\onlyThursday\\total_encoded_Thursday.csv")
    # 获取每个标签的出现次数
    label_counts = df_temp['Label'].value_counts()
    print(label_counts)
    label_counts = df_temp['DestinationPort'].value_counts()
    print(label_counts)
    ...
    # 将更新后的DataFrame保存到文件
    SaveDataToCsvfile(extracted_df, "./data/onlyThursday","total_encoded_updated_Thursday")
...
ChecktotalCsvFileIsexists(filepath + "\\onlyThursday\\total_original_Thursday.csv")
# Loading datasets after megre complete
mergecompelete_dataset = pd.read_csv(filepath + "\\onlyThursday\\total_original_Thursday.csv")
...
df = pd.read_csv("D:\\Labtest20230911\\data\\onlyThursday\\total_encoded_updated_Thursday.csv")
df['DestinationPort'] = df['DestinationPort'].astype(str)
label_Encoding('DestinationPort',df)
SaveDataToCsvfile(df, "./data/onlyThursday","total_encoded_updated_Thursday")


# Loading datasets after label_Encoding
mergecompelete_dataset = pd.read_csv(filepath + "\\onlyThursday\\total_encoded_updated_Thursday.csv")
...
SaveDataToCsvfile(train_dataframes, "./data/onlyThursday", "train_dataframes_Thursday_20231102")
SaveDataToCsvfile(test_dataframes,  "./data/onlyThursday", "test_dataframes_Thursday_20231102")
SaveDataframeTonpArray(train_dataframes, "train_onlyThursday", "20231102")
SaveDataframeTonpArray(test_dataframes, "test_onlyThursday", "20231102")
# SaveDataToCsvfile(train_half1, "./data", "train_half1_20231101")
# SaveDataToCsvfile(train_half2,  "./data", "train_half2_20231101") 
# SaveDataframeTonpArray(test_dataframes, "test", "_Thursday_20231101")
